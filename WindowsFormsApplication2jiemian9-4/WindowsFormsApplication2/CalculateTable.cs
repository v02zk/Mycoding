...
namespace WindowsFormsApplication2
{
    class CalculateTable
    {
        ...
        public void calTableShowMssing(Table comTable, Table normalTable, DataGridView dataTableView, RichTextBox richTex, string tableName)//coTable为对比表格，normalTable为标准表格
        {
            title = tableName;
        ...
        private int calculateSum(int i, int j, Table table, int col)    //一个主项递归计算（部分递归，效率提高）计算第i行j列主项的第col列值
        {
            int itemRow = i;
            int itemColumn = j;
